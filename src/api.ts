...
import type { Narrow, TupleFlat, UnionToTuple } from './utils.types'
import type {
...
class ParametersBuilder<T extends ZodiosEndpointParameter[]> {
  ...
  >(name: Name, type: Type, schema: Schema) {
    ...
    >([...this.params, { name, type, description: schema.description, schema }])
  }
  ...
  >(type: Type, schemas: Schemas) {
    ...
    return new ParametersBuilder<
      [...T, ...Extract<ObjectToQueryParameters<Type, Schemas>, ZodiosEndpointParameter[]>]
 ...
 * @returns - the api definitions
 */
export function makeCrudApi<T extends string, S extends z.ZodObject<z.ZodRawShape>>(
  resource: T,
  schema: S,
) {
      ...
      parameters: [
        {
          ...
          schema: schema.partial() as unknown as z.ZodType<Partial<Schema>>,
        },
      ],
      // @ts-expect-error
      response: schema,
...
type MapApiPath<Path extends string, Api, Acc extends unknown[] = []> = Api extends readonly [
  infer Head,
  ...infer Tail,
]
  ? MapApiPath<
 ...
 */
export function prefixApi<Prefix extends string, Api extends ZodiosEndpointDefinition[]>(
  prefix: Prefix,
  api: Api,
) {
 ...
 * ```
 */
export function mergeApis<Apis extends Record<string, ZodiosEndpointDefinition[]>>(
  apis: Apis,
): MergeApis<Apis> {
